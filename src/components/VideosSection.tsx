...
import { runFullDriveSync, slugify } from '../driveSyncPipeline';
import {
  Play,
  ExternalLink,
  Search,
...
export const VideosSection: React.FC = () => {
  ...
  const [syncConfig, setSyncConfig] = useState<DriveSyncConfig>(() => {
    try {
      const saved = localStorage.getItem(SYNC_CONFIG_STORAGE_KEY);
  ...
  const handleUnlockAdmin = () => {
    ...
    if (val === 'polio2026' || val === 'admin' || val === 'polio' || val === 'asfand') {
      ...
      triggerHaptic('error');
      setAdminAuthError(isUrdu ? 'غلط پاس ورڈ۔ دوبارہ کوشش کریں۔' : 'Incorrect passcode. Try "polio2026".');
    }
  };
  ...
  // Featured Leader Video: picks the first real video from user's Drive videos
  const featuredLeaderVideo = useMemo(() => {
    return allVideos.find((v) => v.featured && v.fileType === 'video') ||
           allVideos.find((v) => v.fileType === 'video') ||
           null;
  }, [allVideos]);
  ...
  const saveSyncConfig = (cfg: DriveSyncConfig) => {
    setSyncConfig(cfg);
    try {
      localStorage.setItem(SYNC_CONFIG_STORAGE_KEY, JSON.stringify(cfg));
    } catch {}
  };
  ...
  return (
    <section id="communication-videos" className="w-full space-y-5">
      ...
      <div className="saas-card p-4 sm:p-6 border-l-4 border-teal-600 relative overflow-hidden bg-white/95 shadow-sm">
          ...
          <div className="flex items-start sm:items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs text-slate-600 mt-1 max-w-2xl leading-relaxed">
                {isUrdu
                  ? 'گوگل ڈرائیو فولڈرز (HCP، مذہبی اسکالرز، دیگر ویڈیوز) سے مستند مواد براؤز اور فوری ڈاؤنلوڈ کریں۔'
                  : 'Authentic resources organized directly from your Google Drive folders: Health Care Professionals (HCP), Religious Influencers, Other Videos, and Guides.'}
              </p>
            </div>
          </div>

          {/* Action Buttons: Sync and Add are strictly visible only to Admin */}
          <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap">
            {isAdmin ? (
              <>
                ...
                {/* Admin: Direct 1-Click Sync Button */}
                <button
                  type="button"
                  ...
                  title="Synchronize all videos from Google Drive folder"
                >
                  <RefreshCw className={`w-3.5 h-3.5 ${isSyncing ? 'animate-spin' : ''}`} />
                  <span>{isSyncing ? (isUrdu ? 'سنک ہو رہا ہے...' : 'Syncing...') : (isUrdu ? 'گوگل ڈرائیو سنک' : 'Sync Google Drive')}</span>
                </button>

                {/* Admin: Sync Settings Button */}
                <button
                  type="button"
                  onClick={() => {
                    triggerHaptic('light');
                  ...
                  title="Google Drive Auto-Sync Configuration"
                >
                  <Settings2 className="w-4 h-4 text-teal-700" />
                  <span>{isUrdu ? 'سنک سیٹنگز' : 'Sync Settings'}</span>
                </button>
        ...
        {/* Master Source Ribbon: Visible only to Admin */}
        {isAdmin && (
          <div className="mt-4 pt-3 border-t border-slate-200/70 flex flex-col sm:flex-row sm:items-center justify-between gap-2.5 text-xs text-slate-600 bg-slate-50/70 p-2.5 rounded-xl">
            <div className="flex items-center gap-2 min-w-0">
              <span className="font-semibold text-teal-900 flex-shrink-0 flex items-center gap-1">
                ...
                {isUrdu ? 'مرکزی کلاؤڈ سورس:' : 'Master Drive Source:'}
              </span>
              <span className="font-mono text-slate-700 truncate max-w-sm sm:max-w-md text-[11px]">
                ID: {syncConfig.masterFolderId || MASTER_FOLDER_ID}
              </span>
              {syncConfig.lastSyncedAt && (
                <span className="text-[10px] text-slate-400 hidden md:inline">
                  • {isUrdu ? 'آخری بار سنک:' : 'Last Synced:'} {new Date(syncConfig.lastSyncedAt).toLocaleTimeString()}
                </span>
              )}
            </div>
            ...
            <div className="flex items-center gap-3">
              ...
              >
                ...
                <span>{isUrdu ? 'خودکار سنک تفصیلات' : 'Auto-Sync Setup'}</span>
              </button>
              <a
                href={MASTER_FOLDER_URL}
                target="_blank"
              ...
              >
                <span>{isUrdu ? 'گوگل ڈرائیو میں کھولیں' : 'Open in Google Drive'}</span>
      ...
      {/* 2. Featured Leader Spotlight: Dr. Syed Bawar Shah (President PPA KP) */}
      {featuredLeaderVideo && (
        <div className="saas-card p-4 sm:p-5 bg-gradient-to-br from-slate-900 via-teal-950 to-slate-950 text-white rounded-3xl border border-teal-500/40 shadow-md relative overflow-hidden">
          <div className="absolute top-0 right-0 w-80 h-80 bg-teal-500/10 rounded-full blur-3xl pointer-events-none" />

          <div className="relative z-10 flex flex-col lg:flex-row lg:items-center justify-between gap-4">
            <div className="flex items-start sm:items-center gap-3.5 sm:gap-4 max-w-2xl">
              <div
                onClick={() => {
                  triggerHaptic('light');
                  setActiveVideo(featuredLeaderVideo);
                }}
                className="w-16 h-16 sm:w-20 sm:h-20 rounded-2xl overflow-hidden border-2 border-teal-400/40 shadow-lg flex-shrink-0 relative cursor-pointer group"
              >
                <VideoThumbnail video={featuredLeaderVideo} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
              ...
              <div className="space-y-1.5 min-w-0">
                <div className="flex items-center gap-2 flex-wrap">
                  <span className="px-2.5 py-0.5 rounded-full bg-teal-500/30 text-teal-200 text-[10px] font-mono font-bold uppercase tracking-wider border border-teal-400/40 flex items-center gap-1">
                    <Award className="w-3 h-3 text-teal-300" />
                    {isUrdu ? 'نمایاں طبی پیغام' : 'Featured Pediatric Endorsement'}
                  </span>
                  <span className="px-2 py-0.5 rounded-full bg-white/10 text-slate-300 text-[10px] font-mono">
                    {featuredLeaderVideo.duration}
                  </span>
                  {featuredLeaderVideo.driveFileId && (
                    <span className="px-2 py-0.5 rounded-full bg-emerald-500/20 text-emerald-300 text-[10px] font-mono">
                      Drive ID: {featuredLeaderVideo.driveFileId}
                    </span>
                  )}
                </div>

                <h3 className="text-sm sm:text-base font-extrabold text-white leading-snug">
                  {isUrdu ? featuredLeaderVideo.titleUr : featuredLeaderVideo.titleEn}
                </h3>

                <div className="flex items-center gap-2 text-xs text-teal-200 font-semibold flex-wrap">
                  <span>{isUrdu ? featuredLeaderVideo.speakerUr : featuredLeaderVideo.speakerEn}</span>
                  <span>•</span>
                  <span className="text-slate-300 text-[11px]">{isUrdu ? featuredLeaderVideo.designationUr : featuredLeaderVideo.designationEn}</span>
                </div>

                <p className="text-xs text-slate-300 leading-relaxed pt-0.5 line-clamp-2">
                  {isUrdu ? featuredLeaderVideo.summaryUr : featuredLeaderVideo.summaryEn}
                </p>
              </div>
            </div>
            ...
            <div className="flex items-center gap-2 flex-shrink-0 pt-2 lg:pt-0">
              <button
                ...
                className="px-4 py-2.5 rounded-xl bg-teal-500 hover:bg-teal-400 text-slate-950 font-extrabold text-xs flex items-center gap-2 shadow-lg transition transform active:scale-95 cursor-pointer min-h-[40px]"
              >
              ...
              <button
                type="button"
                onClick={() => handleDownload(featuredLeaderVideo)}
                className="px-3 py-2.5 rounded-xl bg-emerald-600 hover:bg-emerald-500 text-white font-bold text-xs flex items-center gap-1.5 shadow-md transition transform active:scale-95 cursor-pointer min-h-[40px]"
                title={isUrdu ? 'براہ راست ڈاؤنلوڈ کریں' : 'Download video directly'}
              >
              ...
              {isAdmin && (
                <button
                  ...
                  onClick={() => {
                    triggerHaptic('light');
                    setEditingVideo(featuredLeaderVideo);
                  }}
                  className="px-3 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white font-semibold text-xs flex items-center gap-1.5 transition cursor-pointer min-h-[40px]"
              ...
              <a
                href={featuredLeaderVideo.driveUrl}
                target="_blank"
                rel="noopener noreferrer"
                className="px-3 py-2.5 rounded-xl bg-white/10 hover:bg-white/20 border border-white/20 text-white font-semibold text-xs flex items-center gap-1.5 transition cursor-pointer min-h-[40px]"
              >
      ...
      {/* 3. Folder Category Grid (Health Care Professionals videos, Health are Religious leaders videos) */}
